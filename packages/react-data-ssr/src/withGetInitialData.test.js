...
describe('withGetInitialData SSR', () => {
  ...
  it('should return the setted data', async () => {
    ...
    const getData = jest.fn().mockImplementation((props, { setData }) => new Promise(rs => {
      setData(data);
      rs();
    }));
    const mapDataToProps = jest.fn();
  ...
  it('should call `generateComponentKey` with mapped props', async () => {
    ...
    const extra = { test: 'test' };
    const getData = jest.fn().mockReturnValue(Promise.resolve());
    const generateComponentKey = jest.fn().mockImplementation((Component, props) => {
...
describe('withGetInitialData Component', () => {
  ...
  it('should not getData if it has been already broght (no double fetch)', () => {
    ...
    mount(<Component getInitialData={jest.fn()} hasLoadedComponent={() => true} dismissLoadedComponent={jest.fn()}/>);
  ...
  it('should pass component key to `hasLoadedComponent`', () => {
    ...
    mount(<Component getInitialData={jest.fn()} hasLoadedComponent={hasLoadedComponent} dismissLoadedComponent={jest.fn()}/>);
  ...
  it('should dismiss data when unmounting', () => {
    ...
    const wrapper = mount(<Component getInitialData={jest.fn()} hasLoadedComponent={jest.fn()} dismissLoadedComponent={dismissLoadedComponent}/>);
  ...
  it('should use `generateComponentKey`', () => {
    ...
    const wrapper = mount(<Component getInitialData={getInitialData} hasLoadedComponent={hasLoadedComponent} dismissLoadedComponent={dismissLoadedComponent}/>);
    wrapper.unmount();
  ...
  it('should not refetch if properties have changed (default behaviour)', () => {
    ...
    const wrapper = mount(<Component getInitialData={jest.fn()} hasLoadedComponent={jest.fn()} dismissLoadedComponent={dismissLoadedComponent} testProp={1}/>);
    expect(dismissLoadedComponent).not.toHaveBeenCalled();
    expect(getData).toHaveBeenCalled();
  ...
  it('should refetch if properties have changed and `shouldGetData` says so', () => {
    ...
    const hasLoadedComponent = jest.fn();
    const dismissLoadedComponent = jest.fn();
    const wrapper = mount(<Component getInitialData={getInitialData} hasLoadedComponent={hasLoadedComponent} dismissLoadedComponent={dismissLoadedComponent} testProp={1}/>);
  ...
  it('should map data to properties with initial data', async () => {
    ...
    mount(<Component getInitialData={() => data} hasLoadedComponent={() => true} dismissLoadedComponent={jest.fn()}/>);
